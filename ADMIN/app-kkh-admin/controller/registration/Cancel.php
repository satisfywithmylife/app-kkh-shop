...
class Registration_CancelController extends APF_Controller
{
...
//      Logger::info(__FILE__, __CLASS__, __LINE__, var_export($res, true));
        ...
        if($bll_user->verify_user_access_token($kkid, $token)){ //验证登录
            ...
            if(true){
               ...
               $bll_registration->cancel_registration_by_kkid($r_kkid, $kkid, $res);

               $registration = $bll_registration->get_registration($r_kkid, $kkid);
               $res = $registration;

               $bll_registration->mail_notifiaction($r_kkid, $registration, 'cancel', CS_MAIL_GROUP, TECH_MAIL_GROUP);
