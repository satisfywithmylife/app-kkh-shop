...
class Dao_Payment_NewsaleInfo {
	...
	public function set_payment_refund_msg_by_id($data) {
        ...
        public function create_payment_charge($data) {
				...
				$sql = "insert into t_payment_charge (pid, kkid, o_kkid, charge_id, charge_created, channel, order_no, client_ip_pp, amount, currency, subject, body, time_paid, time_expire, payment_status, status, created, client_ip)values(:pid, :kkid, :o_kkid, :charge_id, :charge_created, :channel, :order_no, :client_ip_pp, :amount, :currency, :subject, :body, :time_paid, :time_expire, :payment_status, :status, :created, :client_ip);";
                $stmt = $this->pdo->prepare($sql);
        ...
        private function get_payment_kkid_by_hid($pid) {
                $kkid = '';
                $sql = "select `o_kkid` from `t_payment_charge` where `pid` = ? ;";
                $stmt = $this->pdo->prepare($sql);
