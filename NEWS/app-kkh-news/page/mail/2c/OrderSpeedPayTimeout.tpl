...
{block name="style"}
    {include file="2c/OrderSpeedPayTimeout.css"}
{/block}
...
{block name="body"}
    <div class="header">
        ...
        <div class="float-left">
            <h2>{Trans::t("order_speed_pay_timeout_info")}</h2>
        </div>
    ...
    <div class="main_header">
        <p class="order_new_msg font-big">{Trans::t("order_speed_pay_timeout_msg_%gname_%checkindate_%homestayname", null, ['%gname'=>"<b>`$order->guest_name`</b>", '%checkindate'=>$order->guest_date, '%homestayname'=>$homestay['name']])}</p>
    </div>
