...
class Registration_CreateController extends APF_Controller
{
...
//      Logger::info(__FILE__, __CLASS__, __LINE__, var_export($res, true));
        ...
        Logger::info(__FILE__, __CLASS__, __LINE__, "kkid: ".$kkid);
        Logger::info(__FILE__, __CLASS__, __LINE__, "token: ".$token);
        $msg = "normal request";
        $bll_user = new Bll_User_UserInfo();
        if($bll_user->verify_user_access_token($kkid, $token)){ //验证登录
            Logger::info(__FILE__, __CLASS__, __LINE__, "verify_user_access_token pass");
               //add
            if(true){
               $bll_registration = new Bll_Registration_Info();
               $r_kkid = $bll_registration->add_registration($kkid, $res);
...
/*
               $ch = \Pingpp\Charge::create(
                   array(
                       ...
                       'channel'   => 'wx_pub', //wx_pub , alipay
                       'amount'    => 20.0,
                       'client_ip' => $client_ip,
                       'currency'  => 'cny',
                       'subject'   => 'registration',
                       'body'      => 'test request',
                       'extra'     => array('open_id' => 'o_6qLuCD5-BwOt61V391OXDzLrMM'), //o_6qLuCD5-BwOt61V391OXDzLrMM
                   )
               );
*/
/*
...
*/
               ...
               if(!empty($r_kkid)){
                   //$bll_registration->mail_notifiaction($r_kkid, $res, 'new', CS_MAIL_GROUP, TECH_MAIL_GROUP);
               }
