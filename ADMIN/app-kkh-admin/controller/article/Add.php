...
class Article_AddController extends APF_Controller
{
    ...
    {   
		...
		#nesseary data
		if (empty(trim($data['title'])) || empty(trim($data['content'])) ||empty($belong)) {
		...
		//Logger::info(__FILE__, __CLASS__, __LINE__, var_export($belong_arr, true));
		if (!empty($belong_arr)) {
		//	Logger::info(__FILE__, __CLASS__, __LINE__, '===1===');
