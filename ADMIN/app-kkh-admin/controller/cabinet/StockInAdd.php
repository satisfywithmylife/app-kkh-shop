...
class Cabinet_StockInAddController extends APF_Controller {
...
//        $id_cabinet = intval($param['id_cabinet']);
        ...
        $num_in = intval($param['num_in']);

//        if($id_product < 0 || $num_in < 0 || $id_cabinet <= 0) {
        if($id_product < 0 || $num_in < 0) {
            self::$error_ret['msg'] = 'invalid param';
            echo json_encode(self::$error_ret);
            Logger::info(__METHOD__ . ' invalid param, param = ' . json_encode($param));
            return false;
        ...
        $ret = $bll_cabinet->stock_in_add($id_cabinet, $id_product, $num_in);
