...
class Dao_Activity_Luck {
    ...
    public function get_luck_records() {
        ...
        $sql = "select * from a_luck_order where is_luck!=1 order by id desc limit 100";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $tmp_comm = $stmt->fetchAll();
        
        $sql = "select * from a_luck_order where id>3583 and is_luck=1 order by id desc";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
