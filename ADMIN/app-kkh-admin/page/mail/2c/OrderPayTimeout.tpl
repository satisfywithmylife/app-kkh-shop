...
{block name="style"}
    {include file="2c/OrderPayTimeout.css"}
{/block}
...
{block name="body"}
    <div class="header">
        ...
        <div class="float-left">
            <h2>{Trans::t("order_pay_timeout_info")}</h2>
            <p>{Trans::t("order_pay_timeout_tips_%s", null, ['%s'=>"<b>`$order->guest_name`</b>"])}</p>
        </div>
    ...
    <div class="main_header">
        <p class="order_new_msg font-big">{Trans::t("order_pay_timeout_msg_%gname_%checkindate_%homestayname", null, ['%gname'=>"<b>`$order->guest_name`</b>", '%checkindate'=>$order->guest_date, '%homestayname'=>$homestay['name']])}</p>
        <a target="_blank" href="http://www{$apf->get_config('base_domain')}/user/feedback/" id="pay_btn">{Trans::t("i_need_to_complain")}</a>
        <p class="pay_link_tips">{Trans::t("pay_timeout_complain_tips")}</p>
    </div>
