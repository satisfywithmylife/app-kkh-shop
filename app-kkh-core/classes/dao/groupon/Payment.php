...
class Dao_Groupon_Payment {
	...
	public function __construct() {
        ...
        public function create_payment_charge($data) {
                ...
                $sql = "insert into `t_payment_charge` (`pid`, `kkid`, `id_customer_group`, `o_kkid`, `charge_id`, `charge_created`, `channel`, `order_no`, `client_ip_pp`, `amount`, `currency`, `subject`, `body`, `time_paid`, `time_expire`, `payment_status`, `status`, `created`, `update_date`, `client_ip`) values(0, replace(upper(uuid()),'-',''), :id_customer_group, :o_kkid, :charge_id, :charge_created, :channel, :order_no, :client_ip_pp, :amount, :currency, :subject, :body, :time_paid, :time_expire, :payment_status, :status, :created, now(), :client_ip);";
                $stmt = $this->pdo->prepare($sql);
	...
	public function get_payment_charge_list_by_id_customer_group($id_customer_group, $limit, $offset)
        {   
            ...
            $sql = "select `id_customer_group`, `pid`, `kkid`, `o_kkid`, `charge_id`, `charge_created`, `channel`, `order_no`, `client_ip_pp`, `amount`, `currency`, `subject`, `body`, `time_paid`, `time_expire`, `payment_status`, `status`, `created`, `update_date`, `client_ip` from `t_payment_charge` where status = 1 and id_customer_group = :id_customer_group and id_customer_group != 0 order by pid desc LIMIT :limit OFFSET :offset;";
            //Logger::info(__FILE__, __CLASS__, __LINE__, "sql: $sql");
            $stmt = $this->pdo->prepare($sql);
        ...
        public function get_payment_charge_list_paid($o_kkid)
        {
            ...
            $sql = "select `pid`, `kkid`, `o_kkid`, `charge_id`, `charge_created`, `channel`, `order_no`, `client_ip_pp`, `amount`, `currency`, `subject`, `body`, `time_paid`, `time_expire`, `payment_status`, `status`, `created`, `update_date`, `client_ip` from `t_payment_charge` where status = 1 and o_kkid = :o_kkid and payment_status in ( 1 , 3 );";
            Logger::info(__FILE__, __CLASS__, __LINE__, "sql: $sql");
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':o_kkid', $o_kkid, PDO::PARAM_STR);
            $stmt->execute();
        ...
        {   
            ...
            $sql = "select `id_customer_group`, `pid`, `kkid`, `o_kkid`, `charge_id`, `charge_created`, `channel`, `order_no`, `client_ip_pp`, `amount`, `currency`, `subject`, `body`, `time_paid`, `time_expire`, `payment_status`, `status`, `created`, `update_date`, `client_ip` from `t_payment_charge` where status = 1 and id_customer_group = :id_customer_group and payment_status in ( 1 , 3 );";
            Logger::info(__FILE__, __CLASS__, __LINE__, "sql: $sql");
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':id_customer_group', $id_customer_group, PDO::PARAM_INT);
            $stmt->execute();
