...
class Cabinet_CounterAddController extends APF_Controller {
...
//        $id_cabinet = intval($param['id_cabinet']);
        ...
        $add_num = intval($param['num']); // 增加的商品数量

        if ($id_cabinet < 1 || $counter_row < 1 || $counter_column < 1) {
            Logger::info(__METHOD__ . ' invalid param, param = ' . json_encode($param));
            self::$error_ret['msg'] = 'invalid param';
            echo json_encode(self::$error_ret);
            return false;
        ...
        $ret = $bll_cabinet->counter_add($id_cabinet, $counter_row, $counter_column, $add_num);
