<?php
/**
 * add uninit function for child processes reunit the Logger
 *
 * @matiainer baronyuan
 */
...
class Util_New_Logger
{
    ...
    function __construct($logname = '')
    {
        ...
        $this->ip			= $_SERVER['HTTP_X_FORWARDED_FOR']?$_SERVER['HTTP_X_FORWARDED_FOR']:($_SERVER['REMOTE_ADDR']?$_SERVER['REMOTE_ADDR']:'127.0.0.1');
    ...
    /**
     * debug，只在dev环境生效，将变量输出到浏览器的控制台上。
     * 参数可变
     * @author derongzeng
     * @return boolen
     */
    public static function debug() {
        if ('dev' != get_cfg_var('env.name')) {
            return FALSE;
        }

        if (!is_file('/data/dev/PHPLIB/lib/FirePHPCore/fb.php')) {
            return FALSE;
        }
        require_once '/data/dev/PHPLIB/lib/FirePHPCore/fb.php';
        $trc = debug_backtrace();
        $title = end(explode(DIRECTORY_SEPARATOR, $trc[0]['file'])) . ' line' . $trc[0]['line'];
        FB::log('-------------------------------debug ' . $title . ' :');
