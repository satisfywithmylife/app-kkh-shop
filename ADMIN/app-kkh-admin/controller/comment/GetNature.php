...
class Comment_GetNatureController extends APF_Controller {
    ...
    public function handle_request() {
        ...
        if(empty($param) || empty($param['page_num']) || empty($param['page_size'])
            || !isset($param['keyword']) || !isset($param['only_negative']) || !isset($param['have_picture'])) {
        ...
        $product_name = $param['keyword'];
		if(empty($product_name)) {
			$product_name = '';
		}
        $only_negative = $param['only_negative'] === "true" ? 1 : 0;    
        ...
        self::$success_ret['data'] = $bll_comment->getNature($product_name, $only_negative, $have_picture, $page_num, $page_size);
